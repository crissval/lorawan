...
static void RxWindowSetup( bool rxContinuous, uint32_t maxRxWindow )
{
    ...
    {
        lora_radio.Rx( maxRxWindow );
    }
    else
    {
        lora_radio.Rx( 0 ); // Continuous mode
    }
}
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        case FRAME_TYPE_JOIN_REQ:
            ...
            LoRaMacDevNonce = lora_radio.Random( );
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    // Send now
    lora_radio.Send( LoRaMacBuffer, LoRaMacBufferPktLen );
...
LoRaMacStatus_t SetTxContinuousWave1( uint16_t timeout, uint32_t frequency, uint8_t power )
{
    lora_radio.SetTxContinuousWave( frequency, power, timeout );
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks, LoRaMacRegion_t region )
{
    ...
    lora_radio.Init( &RadioEvents );

    // Random seed initialization
    srand1( lora_radio.Random( ) );

    PublicNetwork = true;
    lora_radio.SetPublicNetwork( PublicNetwork );
    lora_radio.Sleep( );
...
void LoRaMacDeInit()
{
  ...
  if(lora_radio.Sleep) { lora_radio.Sleep(); }
...
void LoRaMacCancel()
{
  if(LoRaMacState == LORAMAC_IDLE) { goto exit; }
  ...
  if((LoRaMacState & LORAMAC_TX_RUNNING) && ! IsUpLinkCounterFixed)
  {
      UpLinkCounter++;
  }
...
bool LoRaMacIsBusy()
{
  return LoRaMacState != LORAMAC_IDLE;
}
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    switch( mlmeRequest->Type )
    {
        case MLME_JOIN:
        {
            ...
            verify.DatarateParams.Datarate = mlmeRequest->Req.Join.DataRate;
            if(RegionVerify(LoRaMacRegion, &verify, PHY_TX_DR))
            {
              LoRaMacParams.ChannelsDatarate      = mlmeRequest->Req.Join.DataRate;
            }
            else { LoRaMacParams.ChannelsDatarate = RegionAlternateDr( LoRaMacRegion, &altDr ); }
