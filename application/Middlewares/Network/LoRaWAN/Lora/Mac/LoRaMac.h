...
typedef struct sMlmeReqJoin
{
    ...
    /*!
     * Added By Jérôme FUCHET For ISITE/ConnecSenS.
     *
     * If set to a a valid datarate then this sets the datarate to use for the join request.
     * Else use the standard method to find out the join datarate to use.
     */
    int8_t DataRate;
}MlmeReqJoin_t;
...
/*!
 * \brief De-initialise MAC layer.
 */
void LoRaMacDeInit();

/*!
 * \brief Cancel current running operation, if there is one.
 */
void LoRaMacCancel();

/*!
 * \brief indicate if the MAc layer is busy doing something (not in idle state).
 *
 * @return true  if busy.
